...
abstract class BlockTag extends Tag
{
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		if ($this->mpdf->tableLevel) {
			...
			if ($tag === 'LI') {
				...
				if ($this->mpdf->usingCoreFont) {
					$ls = BlockTag.phpstr_repeat(chr(160).chr(160), ($this->mpdf->listlvl-1)*2).$blt. ' ';
				} else {
					$ls = BlockTag.phpstr_repeat("\xc2\xa0\xc2\xa0", ($this->mpdf->listlvl-1)*2).$blt. ' ';
				}
		...
		if ($bdf) {
			...
			$this->mpdf->OTLdata = [];
			if ($this->mpdf->tableLevel) {
				$this->mpdf->_saveCellTextBuffer(BlockTag.phpUtfString::code2utf($bdf).$bdf2);
			} else {
				$this->mpdf->_saveTextBuffer(BlockTag.phpUtfString::code2utf($bdf).$bdf2);
			}
