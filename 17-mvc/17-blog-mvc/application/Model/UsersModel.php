...
class UsersModel extends BaseModel{
  ...
  public function findAll():array {
    $query=self::$pdo->prepare('SELECT * FROM mvc_users');
    $query->execute();
    return $query->fetchAll(PDO::FETCH_CLASS,'\Blog\Model\Entities\User');
  }
  ...
  public function findById(int $id):User|bool {
    $query=self::$pdo->prepare('SELECT * FROM mvc_users WHERE id=:id LIMIT 1;');
    $query->execute([':id'=>$id]);
    return $query->fetchObject('\Blog\Model\Entities\User');
  }
  ...
  public function findByEmail(string $email):User {
    $query=self::$pdo->prepare('SELECT * FROM mvc_users WHERE email=:email LIMIT 1;');
  ...
  public function delete(User|int $id):bool {
    ...
    $query=self::$pdo->prepare('DELETE FROM mvc_users WHERE id=:id LIMIT 1;');
