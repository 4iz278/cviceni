...
class AclModel extends BaseModel{
  ...
  public function findInheritedRoles(string $role):array {
    $result=[];
    $query=self::$pdo->prepare('SELECT * FROM mvc_roles WHERE `id`=:role;');
  ...
  public function findResourcesByRoles(array $rolesArr):array {
    ...
    $query=self::$pdo->prepare('SELECT * FROM mvc_resources WHERE `role` IN ('.$rolesSql.')');
    $query->execute();
  ...
  public function isAllowed(string $userRole, string $resource, string $action):bool {
    ...
    $query=self::$pdo->prepare('SELECT role FROM mvc_resources WHERE `role` in ('.$rolesSql.') AND resource=:resource AND (`action`=\'\' OR `action`=:action);');
