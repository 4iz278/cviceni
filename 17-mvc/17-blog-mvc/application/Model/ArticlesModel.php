...
class ArticlesModel extends BaseModel{
  ...
  public function findAll(?int $category=null):array {
    if ($category>0){
      $query=self::$pdo->prepare('SELECT * FROM mvc_articles WHERE category=:category;');
      $query->execute([':category'=>$category]);
    }else{
      $query=self::$pdo->prepare('SELECT * FROM mvc_articles');
      $query->execute();
    }
    return $query->fetchAll(PDO::FETCH_CLASS,'\Blog\Model\Entities\Article');
  }
  ...
  public function find(int $id, bool $includeTextAliases=false):Article {
    if ($includeTextAliases){
      $sql='SELECT articles.*,users.name AS authorName,categories.name AS categoryName FROM mvc_articles LEFT JOIN mvc_users ON mvc_articles.author=mvc_users.id LEFT JOIN mvc_categories ON mvc_articles.category=mvc_categories.id WHERE mvc_articles.id=:id LIMIT 1;';
    }else{
  ...
  public function delete(Article|int $id):bool {
    ...
    $query=self::$pdo->prepare('DELETE FROM mvc_articles WHERE id=:id LIMIT 1;');
