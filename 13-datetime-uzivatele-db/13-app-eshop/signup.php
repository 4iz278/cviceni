<?php
  ...
  if (!empty($_POST)) {
    ...
    $stmt = $db->prepare("INSERT INTO e_users(email, password) VALUES (?, ?)");
    ...
    $stmt = $db->prepare("SELECT id FROM e_users WHERE email = ? LIMIT 1"); //limit 1 jen tu jen jako výkonnostní optimalizace
    $stmt->execute([$email]);
