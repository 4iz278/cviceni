<?php
  ...
  $stmt = $db->prepare('SELECT e_goods.*, e_users.email, now() > last_edit_starts_at + INTERVAL 5 MINUTE AS edit_expired FROM e_goods LEFT JOIN e_users ON e_users.id=e_goods.last_edit_starts_by_user WHERE e_goods.id=:id');
  ...
  $stmt = $db->prepare("UPDATE e_goods SET last_edit_starts_at=NOW(), last_edit_starts_by_user=:user WHERE id=:id");
  ...
  if (!empty($_POST)) {
    ...
    if (empty($formErrors)){
      ...
      $stmt = $db->prepare('UPDATE e_goods SET name=:name, description=:description, price=:price, last_edit_starts_by_user=NULL, last_edit_starts_at=NULL WHERE id=:id LIMIT 1;');
      $stmt->execute([
        ':name'=> $name,
