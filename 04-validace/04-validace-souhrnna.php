...
if (!empty($_POST['chyby']) && ($_POST['akce']=='odeslano')){
  ...
  if (!preg_match("/^[a-zA-Z -.]{2,}$/",$_POST['jmeno'])){//preg_match kontroluje pomocí regulárního výrazu
    $chyby[]='Je nutné zadat jméno! Jméno může obsahovat jen písmena, mezeru a pomlčku a mít délku min. 2 znaky.';
  }

  $_POST['prijmeni']=trim($_POST['prijmeni'] ?? '');
  if (!preg_match("/^[a-zA-Z -.]{2,}$/",$_POST['prijmeni'])){
    $chyby[]='Je nutné zadat příjmení! Jméno může obsahovat jen písmena, mezeru a pomlčku a mít délku min. 2 znaky.';
  }
  ...
  if ($_POST['email']!='' && !filter_var($_POST['email'],FILTER_VALIDATE_EMAIL)){//filter_var umožňuje jednoduchou kontrolu základních typů dat (čísla, mail, url atp.)
    $chyby[]='Zadaný e-mail není platný!';
  }elseif ($_POST['phone']!='' && !preg_match("/^\+420[0-9]{9}$/",$_POST['phone'])){
    $chyby[]='Zadaný telefon není platný!';
  }elseif($_POST['email']=='' && $_POST['phone']==''){
    $chyby[]='Je nutné zadat telefon či e-mail!';
  }
  ...
  if (!in_array($_POST['phone-os'],['android','ios','windows'])){
    ...
    $chyby[]='Je nutné vybrat operační systém telefonu!';
  }
...
<h1>Ukázka formuláře s validací 1</h1>
